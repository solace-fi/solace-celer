...
import { expectDeployed, isDeployed } from "./utils";
...
const failDeployer = new ethers.Wallet(process.env.PRIVATE_KEY || '', provider2);
...
async function _deployer(wallet: Signer, initCode: string, salt: string, deployerAddress: string) {
  ...
  try {
    let deployerContract = (await ethers.getContractAt(artifacts.Deployer.abi, deployerAddress)) as unknown as Deployer;
